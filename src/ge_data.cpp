...
bool Data::Load()
{
	...
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_RIGHT,skybox_right,GL_RGBA);
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_LEFT,skybox_left,GL_RGBA);
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_TOP,skybox_top,GL_RGBA);
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_BOTTOM,skybox_bottom,GL_RGBA);
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_FRONT,skybox_front,GL_RGBA);
	if(!res) return false;
	res = LoadImage(IMG_SKYBOX_BACK,skybox_back,GL_RGBA);
	if(!res) return false;
