...
bool Stage::Init(LevelFile::Stage currentStage, GLFWwindow *newWindow)
{
	window = newWindow;
	bool res = true;
	noclip = false;
	...
	scene.LoadLevel(currentStage,&terrain,&shader,&data,&lava);

	// //columns initialization
	std::vector<LevelFile::Column> column_configs = currentStage.get_data().get_columns();
	Column col;
	...
	{
		LevelFile::RespawnPoint respawnPoint_data = respawnPoint_list.at(i);
		LevelFile::Coord respawnPoint_coord = respawnPoint_data.get_coord();
	...
	LevelFile::Portal portal_data = currentStage.get_data().get_portal();
...
void Stage::Physics(Bicho &object)
{
